...
import { createTranscriptProcessor } from "../src/transcripts";
import { ensureDirectoryExists } from "../src/utils";

async function main() {
  clack.intro("Transcript Processor");
  ...
  // Success message
  clack.outro(
    `All transcripts processed successfully! Check the ${ansis.cyan(DATA_DIR)} directory for results.`
  );
}
